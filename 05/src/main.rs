...
mod test {
    ...
    fn examples() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(1, Intcode::from_str("3,9,8,9,10,9,4,9,99,-1,8")?.run_input(&[8]).out()[0]);

        assert_eq!(1, Intcode::from_str("3,9,7,9,10,9,4,9,99,-1,8")?.run_input(&[1]).out()[0]);
        assert_eq!(0, Intcode::from_str("3,9,7,9,10,9,4,9,99,-1,8")?.run_input(&[8]).out()[0]);

        assert_eq!(0, Intcode::from_str("3,3,1108,-1,8,3,4,3,99")?.run_input(&[9]).out()[0]);
        assert_eq!(1, Intcode::from_str("3,3,1108,-1,8,3,4,3,99")?.run_input(&[8]).out()[0]);

        assert_eq!(0, Intcode::from_str("3,3,1107,-1,8,3,4,3,99")?.run_input(&[8]).out()[0]);
        assert_eq!(1, Intcode::from_str("3,3,1107,-1,8,3,4,3,99")?.run_input(&[0]).out()[0]);

        assert_eq!(0, Intcode::from_str("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9")?.run_input(&[0]).out()[0]);
        assert_eq!(1, Intcode::from_str("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9")?.run_input(&[-1]).out()[0]);

        assert_eq!(0, Intcode::from_str("3,3,1105,-1,9,1101,0,0,12,4,12,99,1")?.run_input(&[0]).out()[0]);
        assert_eq!(1, Intcode::from_str("3,3,1105,-1,9,1101,0,0,12,4,12,99,1")?.run_input(&[-1]).out()[0]);
