...
fn star1() -> Result<usize, Box<dyn Error>> {
    ...
    let layer = img.pix.chunks(ls).min_by_key(|l| l.into_iter().filter(|&&p| p == 0).count()).unwrap();
...
fn merge_layer(upper: Vec<u8>, lower: &[u8]) -> Vec<u8> {
    upper.into_iter().zip(lower).map(|(u,&l)| if u == 2 { l } else {u}).collect()

}

fn print_layer(l: &[u8]) {
    for line in l.chunks(25) {
        println!("{:?}",line.iter().map(|&c| if c == 0 {' '} else {'#'}).collect::<String>());
    }
}
